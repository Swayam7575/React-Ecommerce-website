...
const CrossDrive = () => {
    ...
    const [whatsNewShow, SetWhatsNewShow] = useState(false);

    const [highTOLOW, SethighTOLOWShow] = useState(false);

    const [lowTOHigh, SetLowTOHigh] = useState(false); //change from bodyShowALl, SetBodyWashSHow
    ...
    var bodyLotionProducts = store.filter(product => product.type.includes('cdwhatsnew'));

    var bodyScrubProducts = store.filter(produt => produt.type.includes("cdhighCost"));

    var lowTOHighProducts = store.filter(product => product.type.includes("cdlowtohigh"));
    ...
    return (
        <div className='u20MainParent fof '>
            ...
            <div className='u20HeadingHold gap-20 justify-center relative flex flex-col'>
                <p className='u20Heading'> CrossDrive Tyres </p>
                <p className='u20Desc'> "Experience Effortless Elegance on all Terrains"</p>
            </div>
            ...
            <div className='u20BreadCrumbHold absolute text-sm'>
                <Breadcrumb spacing='8px' separator={<ChevronRightIcon />}>
                    ...
                    <BreadcrumbItem>
                        <Link to={`/CrossDrive`} href='#'>Cross Drive</Link>
                    </BreadcrumbItem>
                </Breadcrumb>
            </div>
            ...
            {allShow && <div className="flex u20prodsHold flex-wrap relative top-96 justify-center text-center">
                {store.map((item) => {
                    ...
                    if (item.type.includes("cdwhatsnew","cdhighCost","cdlowtohigh")) {
                        return (
